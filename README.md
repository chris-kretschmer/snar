...
- **Darstellung** – Admins stellen unter „Admin-Einstellungen → Darstellung“ für alle Nutzer den Namen der Instanz (Seitenleiste, Anmeldeseite, Tab-Titel) und die Akzentfarbe ein (Links, aktive Navigation, Hover, Fokusrahmen, Diagramme). Live-Vorschau, der Kontrast der Farbe auf Weiß muss mindestens 4,5:1 erreichen, „Auf Standard zurücksetzen“ stellt das Original wieder her.
...
- **Updates und Datenbank**: Änderungen am Datenbank-Schema laufen als nummerierte Migrationen (`src/migrations.js`) beim Start automatisch, in einer Transaktion. Liegen Daten in der Datenbank, legt snar vorher eine Sicherung `snar-vor-migration-v<N>-<Zeit>.db` neben `snar.db` ab (alte Sicherungen kannst du später löschen). Eine Datenbank, die von einer neueren Version geschrieben wurde, wird nicht geöffnet, ein Downgrade wird nicht unterstützt. Vor jedem Update trotzdem ein Backup ziehen, siehe oben.
